...
def test_CompositionCalculator_4():
    ...
    assert np.allclose(species_frac, expected_species_frac) == True
    ...
    assert all([np.isnan(species_frac[i]) for i in range(len(species_frac))]) == True
